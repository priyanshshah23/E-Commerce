...
class MasterDao {
  ...
  static const String DOCUMENT_STORE_NAME = 'document';
  static const String SPECIALIZATION_STORE_NAME = 'specializations';
  static const String SLOTS_STORE_NAME = 'slots';
  static const String PHARMACIES_STORE_NAME = 'pharmacies';

  // A Store with int keys and Map<String, dynamic> values.
  // This Store acts like a persistent map, values of which are Fruit objects converted to Map
  final _masterStore = stringMapStoreFactory.store(MASTER_STORE_NAME);
  final _documentrStore = stringMapStoreFactory.store(DOCUMENT_STORE_NAME);
  final _specializationStore =
      stringMapStoreFactory.store(SPECIALIZATION_STORE_NAME);
  final _slotsStore = stringMapStoreFactory.store(SLOTS_STORE_NAME);
  final _pharmaciesStore = stringMapStoreFactory.store(PHARMACIES_STORE_NAME);
  ...
  Future deleteAllSlotItems() async {
    await _slotsStore.delete(await _db);
  }

  Future deleteAllPharmacyItems() async {
    await _pharmaciesStore.delete(await _db);
  }

  Future deleteAllSpecializationItems() async {
    await _specializationStore.delete(await _db);
  }
  ...
  Future deleteSpecialization(List<String> ids) async {
    if (ids != null && ids.length > 0) {
      for (String str in ids) {
        await _specializationStore.record(str).delete(await _db);
      }
    }
  }
