...
class CarouselState extends State<Carousel> {
  ...
  Widget build(BuildContext context) {
    ...
    double left = [DotPosition.topLeft, DotPosition.bottomLeft]
        ...
        : null;
    double right = [DotPosition.topRight, DotPosition.bottomRightCaro]
            .contains(widget.dotPosition)
        ? widget.dotHorizontalPadding
        : null;
...
class DotsIndicator extends AnimatedWidget {
  ...
  Widget _buildDot(int index) {
    ...
    return Container(
      width: dotSpacing + (zoom > 1.0 ? dotWidth * zoom : dotSize * zoom),
      child: Center(
        child: Container(
          decoration: BoxDecoration(
            borderRadius: borderRadius
                ? BorderRadius.all(
                    radius != null ? radius : Radius.circular(getSize(8)))
                : null,
            color: dotColor,
          ),
          width: zoom > 1.0 ? dotWidth * zoom : dotSize * zoom,
          height: dotSize * zoom,
          child: InkWell(
